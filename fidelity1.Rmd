...
Thus, I chose indices: S&P500(^GSPC),NASDAQ Composite(^IXIC),  ETF: MSCI Emerging Markets(EEM), VanEck Vectors Morningstar Wide Moat ETF(MOAT), JPMorgan Emergin Markets Equity Fund Class A(JFAMX).
...
log <- data.frame(cbind(FIVFX_r, EEM_r, MOAT_r, JFAMX_r, GSPC_r, IXIC_r))
...
ggplot(com)+
  ...
  geom_line(aes(x=Date, y=1000*(IXIC/com$IXIC[1]), color = "blue"))
...
```{r}
fit <- lm(FIVFX_r ~ EEM_r + JFAMX_r + MOAT_r + GSPC_r + IXIC_r, log)
summary(fit)
```